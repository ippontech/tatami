...
function makePieChartsList(data, dest) {
    ...
    $.each(data, function (entryIndex, entry) {
        values.push(entry['statusCount']);
        labels.push('@' + entry['username']);
...
//    chart.draw(dt, { 'title':"Today's status sharing", 'width':dest.width(), 'height':dest.height() });
        ...
        var r = Raphael(document.getElementById(dest.attr('id')),width,height);
       // r.pieChart(width/2,height/2,(height/2)-40, [25,25,50],["25","25","50"], "#fff");
    r.pieChart(width/2,height/2,(height/2)-40,values,labels, "#fff");
