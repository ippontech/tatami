...
public class AccountController {
    ...
    @RequestMapping(value = "/rest/account",
            method = RequestMethod.PUT)
    @ResponseBody
    public User updateUserProfileRest(@RequestBody User updatedUser, HttpServletResponse response) {
        User currentUser = authenticationService.getCurrentUser();
        currentUser.setFirstName(updatedUser.getFirstName().replace("<", " "));
        currentUser.setLastName(updatedUser.getLastName().replace("<", " "));
        currentUser.setJobTitle(updatedUser.getJobTitle().replace("<", " "));
    ...
    @RequestMapping(value = "/rest/account",
            method = RequestMethod.DELETE)
    public void suppressUserProfileRest() {
        User currentUser = authenticationService.getCurrentUser();
        if (log.isDebugEnabled()) {
            log.debug("Suppression du compte utilisateur : " + currentUser);
        }
    ...
    @RequestMapping(value = "/rest/preferences",
            method = RequestMethod.GET,
            produces = "application/json")
    @ResponseBody
    public Preferences getPreferencesRest() {
        if (this.log.isDebugEnabled()) {
            this.log.debug("REST request to get account's preferences");
        }
        User currentUser = authenticationService.getCurrentUser();
    ...
    @RequestMapping(value = "/rest/preferences",
            ...
            produces = "application/json")
    @ResponseBody
    public Preferences updatePreferencesRest(@RequestBody Preferences newPreferences, HttpServletResponse response) {
        if (this.log.isDebugEnabled()) {
            this.log.debug("REST request to set account's preferences");
        }
        Preferences preferences = null;
    ...
    @RequestMapping(value = "/rest/password",
            ...
            produces = "application/json")
    @ResponseBody
    public void isPasswordManagedByLDAPRest(HttpServletResponse response) {
        User currentUser = authenticationService.getCurrentUser();
        String domain = DomainUtil.getDomainFromLogin(currentUser.getLogin());
        if (userService.isDomainHandledByLDAP(domain)) {
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
    ...
    @RequestMapping(value = "/rest/password",
            ...
            produces = "application/json")
    @ResponseBody
    public String setPreferencesRest(@RequestBody UserPassword userPassword, HttpServletResponse response) {
        if (this.log.isDebugEnabled()) {
            this.log.debug("REST request to set account's password");
        }
        try {
