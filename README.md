...
2. create-tables.cql
4. all cql/migration/\*.cql files in alphabetical order

Some explanations:
- `create-tables.cql` is the initial bootstrap script. You shouldn't add many database changes after being in production here
- `*.cql` are all scripts altering the schemas or data (including `*_added_entity_*.cql` files generated by `jhipster:entity`). This is where you will add all the database changes
